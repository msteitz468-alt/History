...
**Activation behaviour (per user preference):** Activation must remain lightweight and silent. At the very start of a task-oriented session, emit one minimal, non-intrusive confirmation (e.g. a single internal note or one-sentence status if the user explicitly asks): "Task-observer activated (config + description match). Observations logged silently; surfaced at end-of-session or on request."

Do **not** launch persistent background monitors, print banners, or add visible ceremony unless the user specifically requests visibility. Loud or persistent activation creates noise and contradicts explicit feedback that activation "should NOT be made a big visible deal."
...
**Activation visibility rule (updated from user feedback):** Keep activation
quiet and functional. The only visible output is the minimal confirmation
sentence at session start if the user has the config trigger, or on explicit
request ("Is task-observer active?"). Never auto-spawn monitors or add
ceremonial log entries.
...
### Simple Status Self-Check

If the user asks mid-session "Is task-observer active right now?" or invokes
the skill explicitly, respond with a short status: "Task-observer is active.
OPEN observations in log: N. Relevant to current work: [summary if any]."
This satisfies the need for verifiability without persistent noise.
...
### Self-Enforcement (visible checklist)

This skill practises what it preaches and makes the checklist prominent.

Before surfacing observations at end of session (or when asked for status), the agent **must** internally verify this checklist:
...
If any check fails, correct before finalizing the log entry or summary. This checklist is part of the mandatory pre-logging discipline.
