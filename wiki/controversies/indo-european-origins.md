---
title: Origins of the Indo-Europeans (Homeland and Dispersal)
dispute_type: interpretation
period_involved: [Neolithic, Chalcolithic, Early Bronze Age, Middle Bronze Age]
regions_involved: [eurasian-steppe, anatolia, near-east, central-asia, europe, south-asia, iran-plateau, balkans]
positions:
  - "Steppe / Kurgan hypothesis (Gimbutas → Anthony): PIE homeland on the Pontic-Caspian steppe ~4500–3000 BCE; spread by mobile pastoralists with horses and wheeled vehicles"
  - "Anatolian / farming-dispersal hypothesis (Renfrew): PIE spread with agriculture out of Anatolia from ~7000 BCE, by demic diffusion of farmers, not conquest"
  - "Indo-Hittite model (Sturtevant; Albright & Lambdin in CAH): Anatolian is a coordinate sister branch that split very early, implying a deeper, slower differentiation and an unsettled homeland"
resolution_status: partially-resolved
last_updated: 2026-06-11
tags: [controversy]
---

## The Dispute

Where and when did the speakers of **Proto-Indo-European (PIE)** — ancestor of a
family stretching from Sanskrit and Persian to Greek, Latin, Celtic, Germanic and
the Anatolian languages — originate, and by what mechanism did the family spread
across Eurasia? This is one of the **standing controversies** mandated by
CLAUDE.md and one of the oldest live debates in prehistory, bearing directly on
[[migration-in-human-history|prehistoric migration]], the
[[demic-vs-cultural-diffusion|demic-vs-cultural diffusion]] question, and the
peopling of Bronze Age Europe and South Asia. It is the Indo-European counterpart
to the wiki's [[afroasiatic-homeland]] and [[african-language-family-origins]]
debates.

## Position 1 — Steppe / Kurgan Hypothesis

Associated with **Marija Gimbutas** (1950s–70s) and, in its modern form, **David
Anthony** (*The Horse, the Wheel, and Language*, 2007):

- The PIE homeland was the **Pontic-Caspian steppe** (~4500–3000 BCE); the
  archaeological correlate is the Yamnaya and antecedent Kurgan cultures.
- Dispersal was driven by **mobile pastoralism**, the domesticated **horse**, and
  **wheeled vehicles**, enabling rapid, partly migratory expansion across Europe
  and into Central and South Asia. Links to [[pastoral-nomadism]] and the
  [[eurasian-steppe]].
- Now strongly supported by **ancient DNA** (Haak, Reich, Allentoft 2015): a
  massive Yamnaya-derived genetic influx into Central Europe ~3000–2500 BCE
  (Corded Ware) and a steppe component in South Asia. This evidence, post-dating
  the other positions, has moved much of the field toward the steppe model.

## Position 2 — Anatolian / Farming-Dispersal Hypothesis

Associated with **Colin Renfrew** (*Archaeology and Language*, 1987):

- PIE spread out of **Anatolia** with the [[early-agriculture-southwest-asia|Neolithic
  agricultural expansion]] from ~7000 BCE, carried by farmers via **demic
  diffusion** rather than by mounted conquerors.
- Attractive because it ties language spread to a demonstrable demographic
  process (farming) rather than to elusive invasions; criticized for being too
  early to fit the linguistic reconstruction (PIE vocabulary for wheels and wool
  implies a 4th-millennium, not 7th-millennium, community).
- Some Bayesian phylogenetic studies (Gray & Atkinson 2003; Bouckaert 2012) have
  supported an Anatolian-age origin, keeping the position alive against the aDNA
  tide; hybrid models now place an ultimate deep origin south of the Caucasus with
  a steppe staging area.

## Position 3 — The Indo-Hittite Model (the CAH's 1970 framing)

[[edwards-cah-v1p1-1970|CAH Vol. I, Part 1]] (Ch. IV, Albright & Lambdin) records
an important earlier state of the debate:

- It works within **E. H. Sturtevant's "Indo-Hittite"** framework: the **Anatolian
  group** (recovered after the decipherment of cuneiform Hittite in 1915) is so
  divergent — e.g. it lacks verbal *Ablaut* — that it is best treated as a
  **coordinate sister branch** that separated very early, implying "a long period
  of slow Indo-European development."
- Albright **rejects the equestrian-nomad picture**: it is "erroneous" to say that
  "during the third millennium the Indo-Europeans were nomad horsemen," a claim he
  attributes to "false a priori hypotheses and misdated stratigraphic finds,"
  noting that large-scale horseback riding and spoked-wheel chariots came only late
  in the 2nd millennium. This is a direct early challenge to the mechanism at the
  heart of Position 1.
- He also doubts that the Indo-Hittites **migrated into** Anatolia at the start of
  the 2nd millennium, arguing Hittite was already sharply distinct much earlier —
  leaning, in effect, toward a long Anatolian presence.
- Tellingly, on the central question the chapter punts: "There has been much
  debate as to the original home of the Indo-Europeans, but this matter does not
  concern us here."

## The Anatolian Branch on the Ground (CAH XVIII)

A separate strand of the same problem is the **archaeological** arrival of the
Anatolian languages, addressed by **James Mellaart** in
[[edwards-cah-v1p2-1971|CAH Vol. I, Part 2]] (Ch. XVIII):

- Mellaart read the **end-EB2 catastrophe** in Anatolia (c. 2300 BCE) — a collapse
  from ~450 settlements to ~125, with burn-layers at [[troy|Troy II]],
  Poliochni, Beycesultan and elsewhere — as the trace of an **invasion of
  Indo-European speakers** he identified with the **Luwians**, coming across the
  Dardanelles from the Balkans/Pontic region.
- His cited evidence: a hieroglyphic-Luwian-looking **seal from Beycesultan** and
  the obelisk at [[byblos]] inscribed for "**Kukun son of the Lukka-man**" (Lukka =
  Luwians). See [[early-bronze-age-anatolia]] for the full horizon.
- This is a classic **migrationist** argument and cuts across the homeland
  positions above: a destruction-by-invasion at the *end* of EB2 fits neither a
  slow 7th-millennium farming diffusion (Position 2) nor sits comfortably with the
  very early Indo-Hittite split (Position 3), and modern scholars caution that
  burnt layers need not mean ethnic newcomers. It nonetheless remains one of the
  oldest concrete attempts to date the **Anatolian branch's** appearance from
  material evidence rather than from linguistic reconstruction alone.

## Crossland's Reassessment (CAH XXVII) — the 1971 state of the question

A second, fuller treatment in [[edwards-cah-v1p2-1971|CAH Vol. I, Part 2]] is
**R. A. Crossland's** "Immigrants from the North" (Ch. XXVII), which is valuable as
a careful, **deliberately anti-romantic** statement written just before calibration
and DNA. Its positions:

- **Linguistic, not racial.** Crossland strips out the "Aryan race" and the image of
  "northern virility sweeping down in massed chariots." Same language ≠ same culture
  or physique; the very existence of the Indo-Europeans "as a distinct and coherent
  people" had, he notes, been questioned.
- **Mechanism — the horse, not the chariot.** Like Albright (Position 3) he holds
  the **war-chariot came late** (early 2nd millennium, probably from the
  Hurrian/Iranian zone); the early advantage was simply the **horse** for raiding
  and rapid movement. Their lasting contribution was **political** (state-formation:
  the Hittite kingdom; "Aryan" leadership of the Hurrians) — "no important practical
  innovation … except the use of the horse."
- **Homeland — Pontic-leaning but agnostic.** He surveys **Gimbutas's** rapid
  Kurgan dispersal (battle-axes, ochre-grave *kurgans*, Corded Ware) and **Merpert's**
  longer Lower-Volga "Pit Grave" chronology — preferring Merpert's as a better fit
  for the linguistic differentiation — and **rejects Thieme's** north-central-European
  homeland. His verdict: the homeland is **"still unidentified,"** but the **Pontic
  steppe** is the most probable area of origin for the movements of ~2500–2000 BCE.
- **A dialectology argument for an early Anatolian split.** Crossland reads the
  archaic features shared by **Hittite-Luwian, Italic, Celtic and Tocharian** (the
  *-r* medio-passive; *kwi-* relatives; weak feminine gender) as **"peripheral
  archaisms"** — branches carried out of the continuum *first* — while Greek,
  Indo-Iranian, Germanic, Baltic and Slavonic stayed in contact and innovated (the
  *centum→satem* shift being a late, central change). This **radial-dispersal**
  model points to a centrally placed homeland and an *early* Anatolian divergence —
  effectively an early cousin of today's "**Indo-Anatolian**" phylogenies and a
  linguistic echo of the Indo-Hittite framing in Position 3.
- **Greece ~1900 BCE.** He accepts the standard date for the first Greek-speakers
  (Middle Helladic, Minyan ware) while noting the [[lerna|Lerna]] evidence pushes
  part of the process back to the end of EH II — see
  [[coming-of-the-greeks-1900bce]]. The whole treatment is captured in
  [[indo-european-migrations|the migrations process page]].

Crossland thus sits **between** the modern steppe consensus (he leans Pontic) and
the cautious Indo-Hittite tradition (he takes the early Anatolian split seriously),
and is a strong early voice **against** reading destruction layers — like the
[[early-bronze-age-anatolia|end-EB2 "Luwian" horizon]] — as automatic proof of
ethnic invasion.

## Garašanin's Balkan View (CAH III.1) — the steppe model from the receiving end

[[boardman-cah-v3p1-1982|CAH Vol. III, Part 1]] (Ch. 3, M. Garašanin) adds a
**Balkan archaeologist's** voice, viewing the question from the lands the steppe
peoples entered rather than from Anatolia or the steppe itself:

- He treats the **Eneolithic migrations from the Russian steppes and the Pontic
  basin** — the **"Kurgan culture"** (named for its *kurgan* tumulus burials, ochre
  graves, corded ware and stone battle-axes) — as the archaeological correlate of
  **Indo-Europeanization**, "decisive for the further development of society in
  these regions." This is essentially **Gimbutas's Kurgan model** stated as fact by
  a contemporary working in the target area (Position 1).
- Crucially, he frames the outcome as **assimilation, not replacement**: the
  intruding pastoralists merged with the autochthonous Old European farmers, and
  it is *from this fusion* that the **Palaeo-Balkan Indo-European peoples**
  (Thracians, Illyrians, Daco-Moesians, and ultimately the Greeks) slowly formed —
  "both their languages and they themselves evolved during a long process of …
  assimilation." Ethnogenesis, not invasion.
- He records the **competing linguistic theory** he argues against: that
  Indo-Europeans were in the Balkans **much earlier**, from the Early Neolithic,
  inferred from a deep Indo-European layer in Balkan **toponymy and hydronymy**.
  Garašanin rejects this (no fixed rate of language formation; oldest place-names
  need not match oldest population), placing IE arrival firmly in the **Eneolithic**
  via the steppe. This is the **same early-vs-late tension** that, on the Anatolian
  side, separates Renfrew's farming model (Position 2) from the steppe model.

Garašanin thus supplies, decades before ancient DNA, a clear **steppe-migration +
local-assimilation** account for *Europe's* Indo-Europeanization — the very process
(Yamnaya influx into Corded Ware Europe) that aDNA would later confirm. His
"pots = peoples" method is dated, but his **conclusion** has aged well. See the
[[indo-european-migrations|migrations process page]] (European/Balkan branch) and
[[balkans]].

## Where Things Stand

**Partially resolved, tilting to the steppe.** Ancient DNA since 2015 has given the
Steppe/Kurgan model decisive new support for the *European* and *South Asian*
expansions, and the field's centre of gravity has shifted accordingly. But the
*ultimate* origin and the place of Anatolian remain open: the early, deep split of
Anatolian that the 1970 Indo-Hittite framing captured has resurfaced in modern
"Indo-Anatolian" phylogenies that posit a homeland south of or around the Caucasus
with a subsequent steppe phase. The CAH chapter is valuable here less as authority
than as evidence that the mechanism debate (mounted pastoralists vs. slow
diffusion) and the Anatolian-branch puzzle are a century old.

## Related

- [[afroasiatic-homeland]] — the parallel debate for the Afro-Asiatic family.
- [[demic-vs-cultural-diffusion]] — the underlying "people or ideas?" question.
- [[historical-linguistics-and-agriculture]] — language families as historical evidence.
- [[migration-in-human-history]], [[eurasian-steppe]], [[pastoral-nomadism]].
- [[early-bronze-age-anatolia]] — the end-EB2 "Luwian" destruction horizon.
- [[indo-european-migrations]] — the migrations as a process (Crossland, CAH XXVII).
- [[coming-of-the-greeks-1900bce]] · [[lerna]] — the Greek arrival and its key site.
