---
title: Early Prehistory
period_number: 2
preceded_by: [ [[deep-prehistory|Deep Prehistory]]]
followed_by: [ [[late-prehistory|Late Prehistory]]]
date_range: "3.3 million–300,000 BP"
regions_covered: [africa, near-east, europe, east-asia, south-asia]
major_themes: ["Homo genus evolution", "Oldowan and Acheulean technologies", "Out of Africa 1", "fire management origins", "brain size increase", "Homo heidelbergensis"]
collection_coverage: moderate
sources_ingested: 4
last_updated: 2026-07-08
tags: [period]
---

## Overview

Early Prehistory covers the ~3 million years during which the *Homo* genus
spread from Africa across Eurasia, developed progressively more sophisticated
stone tool technologies, began managing fire, and gradually evolved toward the
anatomically modern form. It closes with the appearance of the first archaic
modern humans and Neanderthals — around 300,000 BP.

This is the period of *Homo habilis*, *Homo ergaster*, *Homo erectus*, and
*Homo heidelbergensis*. Cumulative cultural learning is visible in the
archaeological record but slow; major tool traditions persist for hundreds of
thousands of years with little change.

## Major Developments

**Homo habilis and the Oldowan industry (~2.6–1.7 million BP)**
The first stone tools (Oldowan) are associated with Homo habilis and found
from ~2.6 million BP (notably at Olduvai Gorge, Tanzania — identified by
Louis Leakey in the 1930s). These simple flaked cobbles enabled butchering
of large animals and access to meat and marrow. This dietary shift may have
driven increased brain size and opened new ecological niches in colder regions.

**Out of Africa 1 (~1.8 million BP)**
The first migration of the *Homo* genus out of Africa. See [[out-of-africa-1-1800kbp]].
The earliest dated evidence outside Africa is at Dmanisi, Georgia (1.77 million BP).
Homo remains appear in Java (~1.5 million BP) and China (~1.66 million BP).
European evidence: Atapuerca, Spain (~1.3 million BP). Range initially limited
to approximately 40°N latitude.

**Acheulean technology (~1.7–0.3 million BP)**
Hand axes and other bifacially worked tools appear, associated with
*Homo ergaster* and *Homo erectus*. More versatile than Oldowan tools;
distributed across Africa, Europe, and South/East Asia.

**Lower Palaeolithic Subsistence Evidence in Europe (Champion et al. 1984, Ch. 3)**
Associations of large mammals such as straight-tusked elephant and rhino with
stone tools at marsh, lake and stream deposits (Torralba, Bilzingsleben, Hoxne,
Swanscombe) have led to reconstructions of big-game hunting. The 2.5 m wooden
spear with fire-hardened point at Lehringen, north Germany, provides
unambiguous evidence for killing an elephant. "However, the depositional context
of these finds must not be forgotten and, as we have already seen at Torre in
Pietra (Piperno and Biddittu, 1978), river sorting can produce comparable results
to big-game hunting by gathering material together." (Champion et al. 1984, Ch. 3
/ range lines ~710-730)

Use-wear analysis on lower palaeolithic artifacts from Hoxne and Clacton also
points to meat and marrow being obtained and eaten. Early colonizers of Europe
likely used flexible spectra of food management strategies constrained by the
importance of animal protein for mobile non-food-producing populations in
northern latitudes, rather than exhibiting dramatic changes unique to big-game
hunting. "As we learn more about the subsistence strategies of the earliest
colonizers of Europe, it is most probable that, instead of reconstructing
dramatic changes in terms of developments in big-game hunting, we shall instead
see variation in the spectrum of food management strategies..." (Champion et al.
1984, Ch. 3 / range lines ~750-760)

**Fire management**
The earliest firm evidence for controlled use of fire by hominins dates from
~250,000 years ago, though some archaeologists argue for evidence from
South Africa and Israel as early as 800,000 BP. If the later date is correct,
fire management begins in Late Prehistory; if the earlier date, it begins in
this period. The question remains open (see [[homo-sapiens-emergence-date]] for
the wider dating controversy context). See also: [[processes/fire-management]].

**Initial peopling of Europe (~1.3 million–600,000 BP)**
The first European phase (Hoffecker, Ch. 16) involved sporadic occupation of
southern Europe by earlier *Homo* with pebble/flake tools: Atapuerca, Spain
(jaw + teeth ~1.2–1.3 million BP; younger fossils ~0.8 million BP classified as
*Homo antecessor*, possibly unique to Europe); Ceprano, Italy (skullcap
~0.7–1.0 million BP); a handaxe at Estrecho del Quípar, Spain (~0.9 million BP).

**Homo heidelbergensis (~600,000–200,000 BP)**
A new large-brained species (cranial volume averaging >1,100 cc) migrating from
Africa into western Eurasia ~600,000 BP, probably via the Bosporus. *Heidelbergensis*
was the common ancestor of both Neanderthals and *Homo sapiens*, and had mastered
fire control. It manufactured the refined Acheulean handaxe/biface — production
requiring a preconceived design and hierarchically organized steps (not the
reductive earlier toolmaking). Known from Boxgrove, England (~500,000 BP, a "whole
landscape" of butchery sites; a massive tibia hinting cold adaptations had not yet
evolved) and Schöningen, Germany (~400,000 BP, with the earliest known wooden
spears — pine/spruce, carefully shaped from the hardest wood at the base of the
tree). It gave rise to the [[neanderthals|Neanderthals]] in Europe.

## Key Actors

- *Homo habilis* — first identifiable member of the *Homo* genus; Oldowan tools
- *Homo ergaster* / *Homo erectus* — first African and Eurasian dispersal; Acheulean tools
- *Homo heidelbergensis* — immediate ancestor of both Neanderthals and modern humans

## Geographic Scope

Africa (entire period). Eurasia from ~1.8 million BP. No evidence of hominin
presence in Australasia or the Americas during this period.

## Transition

Follows [[deep-prehistory]], beginning with the earliest recognized stone-tool
manufacture (~3.3 Mya) that defines the start of the archaeological record proper.

Closes with the evolution of *Homo sapiens* (anatomically) in East Africa and
Neanderthals in Europe from *Homo heidelbergensis* ancestors, around 300,000–
250,000 BP. This marks the opening of [[late-prehistory]].

## Historiography

**Source quality**: Fossil and stone tool evidence only. Increasingly
supplemented by ancient DNA (aDNA) analysis, which has transformed the field
since the 2010s — notably the sequencing of the Neanderthal genome (2010) and
the identification of the Denisovans (2010, from a Siberian cave).

**Dating methods**: Potassium-argon, argon-argon, uranium-series, electron
spin resonance, thermoluminescence, paleomagnetism. Confidence intervals
typically ±50,000 to ±100,000 years for this period.

**Major debates**: The taxonomy of Homo erectus vs. Homo ergaster (lumpers vs.
splitters); the routes of Out of Africa 1; whether early Eurasian populations
contributed to the *H. sapiens* lineage; the date and origin of fire control.

**Recent revisionism**: Ancient DNA analysis has revealed that non-African
modern humans carry ~1–4% Neanderthal DNA and that some populations carry
Denisovan DNA, suggesting gene flow was ongoing across much of this period —
complicating a clean "Out of Africa" model in favour of a "mostly out of
Africa with some introgression" model.

**Collection coverage**: Moderate from ingested sources. The CWH Vol. 1
Introduction gives an overview but the detail will come from Phase 11 sources
and the actual chapters (Chs. 13–17) of this volume, not yet ingested.

## Collection Coverage Note

Coverage moderate: architectural sketch from CWH Vol. 1 Introduction.
Chapters 13–17 of this volume (Part II) and Phase 11 specialist sources will
substantially expand this page.

**Champion et al. 1984 (Ch. 2 opening) contribution:** Supplies detailed post-radiocarbon treatment of the chronological and climatic frameworks for early European hominins and the sparse fossil record. Deep-sea core V28-238 provides continuous isotope stages with magnetic reversal baseline at 700,000 BP marking lower/middle Pleistocene boundary. "The discontinuous nature of these terrestrial sequences has been further highlighted by the evidence from deep sea cores that do provide a continuous record." Earliest secure European presence placed after ~350,000 BP by absolute dates on fossils; earlier claims (Vallonnet, Isernia) discussed with caution. Fossil material "meagre" compared to East Africa; taxonomic placement (Homo erectus vs. early Homo sapiens) highly contentious. "The few absolute dates for these fossils all indicate that man was present in Europe after 350000 BP." (Champion et al. 1984, Ch.2 / range lines ~1300-1360, ~1800-1815) This source supplies the primary European-specific grounding for the period page's European evidence sections.

## Fagan & Durrani (2020) — Periodization and Toolmaking Note

Fagan & Durrani (2020) redate the origin of stone toolmaking to **~3.3 Ma**
(the Lomekwi 3 industry, Lake Turkana, Kenya, published 2015) — earlier than
the first appearance of the genus *Homo* itself (per Fagan, a possible first
member ~2.8 Ma, tentative; first definite evidence ~2.4 Ma). This breaks the
traditional exclusive link between *Homo habilis* and the earliest stone
tools: Lomekwi 3 predates *Homo* entirely and was likely made by an
australopithecine or *Kenyanthropus*. The **Oldowan** industry (traditionally
credited to *H. habilis*) is dated by Fagan & Durrani to ~2.6 Ma — consistent
with, if at the older end of, this page's existing range — with sites in
Ethiopia and at Olduvai Gorge, Tanzania (classic Leakey excavations, 1960s;
dates 2.2–1.7 Ma). Because Fagan & Durrani define the start of prehistory
itself as coinciding with Lomekwi 3 (see [[../periods/deep-prehistory|Deep
Prehistory]] for the fuller periodization note), their scheme would place the
opening of "Early Prehistory" — the first stone tools — earlier than this
page's current 3.3 Ma start-date convention treats as the *Homo* genus's
first appearance; the wiki's existing period boundaries are retained here,
with this source flagged as an argument for reconsidering where toolmaking
vs. genus origin should mark the line.

## From Tattersall, *The World from Beginnings to 4000 BCE* (2008)

[[tattersall-world-from-beginnings-2008|Tattersall (2008)]] makes this the period of
the genus *Homo*'s emergence and first dispersal. The decisive event is the **abrupt
arrival of the modern body plan** ([[homo-ergaster|*Homo ergaster*]] / Turkana Boy
~1.6 mya) — plausibly a regulatory-gene change — which, not bigger brains or better
tools (cf. small-brained Dmanisi), enabled [[out-of-africa-1-1800kbp|the first exit
from Africa]] and the East Asian [[homo-erectus|*Homo erectus*]] radiation. Acheulean
handaxes (~1.5 mya) postdate both. See [[human-evolution]].
